    ...
    const parameters = useMemo(function _generateParameters() {
        ...
        return Object.keys(modelConfig.parameters)
            .map(function _forEachParameter(paramName) {
                ...
                switch (type) {
                    case "graph":
                        return (<WizardPage key={`auto-parameter-${paramName}`} title={title}>
                            <DrawLineChart
                                label={label}
                                xRange={xRange}
                                yRange={yRange}
        ...
        <Container fluid className="vh-100">
            {modelConfig && (
                <Row>
                    ...
                    <Col xs={12} md={8} lg={9}>
                        <ModelView modelConfig={modelConfig} setModelConfig={updateModelConfig} />
                    </Col>
                </Row>
            )}
        </Container >
